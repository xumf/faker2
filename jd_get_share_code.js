...
function getCFD(showInvite = true) {
  function taskUrl(function_path, body) {
    return {
      url: `https://m.jingxi.com/jxbfd/${function_path}?strZone=jxcfd&bizCode=jxcfd&source=jxcfd&dwEnv=7&_cfd_t=${Date.now()}&ptag=138631.26.55&${body}&_ste=1&_=${Date.now()}&sceneval=2&g_login_type=1&g_ty=ls`,
      headers: {
        Cookie: cookie,
        Accept: "*/*",
        Connection: "keep-alive",
            ...
            if (res && res.resultCode === 0) {
              ...
              if (res.resultData.data) {
                ...
                if (userInfo.realName) {
                  console.log(`【京东账号${$.index}（${$.UserName}）金果摇钱树】${userInfo.sharePin}`);
                } else {
